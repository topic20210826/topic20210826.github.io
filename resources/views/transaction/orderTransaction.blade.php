                    ...
                    @foreach ($TransactionPaginate as $Transaction)
                        <tr>
                            <td align="center" colspan="2" class="user-top">
                                <a class="order-buttom" href="/delete/{{ $Transaction->id }}">刪除</a>
                                @if ($Transaction->type == 'B')
                                    <a class="order-buttom" href="/complete/{{ $Transaction->id }}">完成</a>
                                @elseif ($Transaction->type == 'O')
                                    <a class="order-buttom2" disabled="disabled">已完成</a>
                                @endif
                            </td>
                        </tr>
                        <tr>
                            <td class="user-table">
                                使用者名稱：{{ $Transaction->user }}
                            </td>
                            <td class="user-table">
                            </td>
                        </tr>
